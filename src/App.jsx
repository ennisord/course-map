...
const ROW_GAP = 100
const HIGHLIGHT = { border: '#2563eb', text: '#93c5fd', muted: '#3b82f6' }

function getColor(tags, selected) {
  if (selected) return HIGHLIGHT
  if (tags.includes('honours')) return { border: '#b8860b', text: '#ffd700', muted: '#8a6500' }
  if (tags.includes('core')) return { border: '#1a5c2e', text: '#4ade80', muted: '#166534' }
...
function CourseNode({ course, pos, selected, onClick }) {
  const { border, text, muted } = getColor(course.tags, selected)
  return (
    <div
            ...
            return course.prereqs.map(prereqId => {
              ...
              return (
                <path
                  ...
                  stroke={isHighlighted ? HIGHLIGHT.border : getColor(course.tags, false).border}
